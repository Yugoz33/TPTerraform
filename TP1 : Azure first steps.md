...
$ az vm create -g tp1leo -n cli_1 --image Ubuntu2204 --admin-username hugoc --ssh-key-values C:\Users\hugoc\.ssh\cloud_tp1.pub --size Standard_B1ls
...
🌞 Prouvez avec une connexion SSH sur l'IP publique que la VM est 

- toujours pas de password avec votre Agent SSH normalement 

```
$ ssh -i C:\Users\hugoc\.ssh\id_tpleo hugocTerraform@**.**.**.**
The authenticity of host '**.**.**.** (**.**.**.**)' can't be established.
